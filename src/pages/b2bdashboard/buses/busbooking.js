...
const BusBooking = () => {
  ...
  const [currentPage, setCurrentPage] = useState(
    Number(sessionStorage.getItem("bus_p")) || 1
  );
  ...
  useEffect(() => {
    sessionStorage.setItem("bus_p", currentPage.toString());
  }, [currentPage]);
  // ...

  useEffect(() => {
    const storedPage = Number(sessionStorage.getItem("bus_p")) || 1;
  ...
  const handleClick = (e) => {
    ...
    sessionStorage.setItem("bus_p", pageNumber.toString());
  };
  const pages = [];
  for (let i = 1; i <= Math.ceil(busConfirmation?.length / limit); i++) {
    pages.push(i);
  }
  ...
  const currentItems = busConfirmation?.slice(startIndex, lastIndex);

  

  const renderData = (busConfirmation) => {
    return (
      <>
  ...
  const handlePrevious = () => {
    ...
    sessionStorage.setItem("bus_p", newPage.toString());
