...
const UpdateInternational = () => {
  ...
  useEffect(() => {
    ...
    if (id) {
      fetch(`http://localhost:5000/api/v1/international/${id}`)
        .then((res) => res.json())
        .then((data) => {
  ...
  const handleHotelData = (e) => {
    ...
    setLoading(true);
    axios
      .put(`http://localhost:5000/api/v1/international/${id}`, data)
        ...
        <div className={styling.profileTop}>
          <div className={styling.flightHistory}>
            <h2 className="text-3xl font-bold text-center">
              International Hotel Data Update
            </h2>
...
export default dynamic(() => Promise.resolve(UpdateInternational), { ssr: false });
