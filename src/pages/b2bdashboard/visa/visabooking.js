...
const VisaBooking = () => {
  ...
  const [currentPage, setCurrentPage] = useState(
    Number(sessionStorage.getItem("visa_p")) || 1
  );
  ...
  useEffect(() => {
    sessionStorage.setItem("visa_p", currentPage.toString());
  }, [currentPage]);
  // ...

  useEffect(() => {
    const storedPage = Number(sessionStorage.getItem("visa_p")) || 1;
  ...
  const handleClick = (e) => {
    ...
    sessionStorage.setItem("visa_p", pageNumber.toString());
  };
  const pages = [];
  for (let i = 1; i <= Math.ceil(visaConfirmation?.length / limit); i++) {
    pages.push(i);
  }
  ...
  const currentItems = visaConfirmation?.slice(startIndex, lastIndex);

  const renderData = (visaConfirmation) => {
    return (
      <>
  ...
  const handlePrevious = () => {
    ...
    sessionStorage.setItem("visa_p", newPage.toString());
  ...
  return (
    <B2BdashboardLayout>
      <div className={style.ticketListHead}>
        <h3 className="text-2xl font-bold text-white">Visa Booking Info</h3>
      </div>
...
export default dynamic(() => Promise.resolve(VisaBooking), { ssr: false });
